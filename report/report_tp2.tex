...
\begin{itemize}[leftmargin=1.5em]
  \item An \textbf{OWL 2 DL ontology} that formally describes the F1 domain,
        ...
        the facts file.
  \item \textbf{19 SPIN rules} (SPARQL INSERT WHERE) that materialise
...
The dataset is the \textbf{Kaggle Formula 1 World Championship} dataset
(1950--2024), which follows the Ergast-style CSV schema:
75 seasons, 861 drivers, 212 constructors, 77 circuits, and approximately 25{,}000 race results.
The raw data comes from 13 CSV files loaded into a GraphDB repository named \texttt{ws-formula1-owlmax}.
...
After loading the integrated file and running the 19 SPIN rules, all inference
checks pass:
...
complete ontology (375 triples, 18 classes, 19+ object properties, 42 datatype
...
The 19 rules are defined in \texttt{championship/spin\_rules.py} as a list of
...
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{OWL 2 DL ontology} with 18 classes, 19+ properties, and 7 disjointness
        axioms. Verified in both GraphDB (OWL-Max ruleset) and Protégé (HermiT 1.4.3, 838~ms).
  \item \textbf{19 SPIN rules} covering all classification and inference patterns
        that OWL DL cannot express. All 19 pass with verified real-world counts.
...
# 7. Run all 19 SPIN inference rules (~1 min)
python3 manage.py run_spin_rules
# Expected output: 19/19 rules applied successfully.
...
\texttt{championship/spin\_rules.py}     & 19 SPIN rules as Python module & Yes \\
\bottomrule
\end{tabular}
\end{center}
