...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Formula 1 dataset CSV files in \texttt{data/raw/} --- download from Kaggle:
        \begin{center}
          \url{https://www.kaggle.com/datasets/rohanrao/formula-1-world-championship-1950-2020}
        \end{center}
        Extract the archive and place all \texttt{.csv} files directly in
        \texttt{data/raw/} (13 files: \texttt{circuits.csv}, \texttt{drivers.csv},
        \texttt{races.csv}, \texttt{results.csv}, etc.).
        The dataset follows the Ergast API schema and is updated after each season.
\end{itemize}
...
Open \url{http://localhost:9000}. The admin panel is at
...
These credentials are set by the \texttt{DJANGO\_SUPERUSER\_USERNAME} and
\texttt{DJANGO\_SUPERUSER\_PASSWORD} variables in \texttt{.env} and created
automatically by \texttt{setup.sh} on first run.
